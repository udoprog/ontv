...
use anyhow::Result;
use api::{ImageHash, ImageHint, ImageV2, ThemeType};
...
pub(crate) struct Assets {
    ...
    missing_poster_dark: image::DynamicImage,
    missing_poster_light: image::DynamicImage,
    /// Handle for missing banners.
    missing_banner: image::DynamicImage,
    /// Handle for missing screen caps.
    missing_screencap: image::DynamicImage,
    ...
    images: HashMap<ImageKey, image::DynamicImage>,
    /// Assets to remove.
...
impl Assets {
    pub(crate) fn new() -> Result<Self> {
        let missing_poster_dark = image::load_from_memory(MISSING_POSTER_DARK)?;
        let missing_poster_light = image::load_from_memory(MISSING_POSTER_LIGHT)?;
        let missing_banner = image::load_from_memory(MISSING_BANNER)?;
        let missing_screencap = image::load_from_memory(MISSING_SCRENCAP)?;
    ...
    pub(crate) fn insert_images(&mut self, loaded: Vec<(ImageKey, image::DynamicImage)>) {
    ...
    pub(crate) fn missing_poster(&self, theme: &ThemeType) -> image::DynamicImage {
        match theme {
            ThemeType::Dark => self.missing_poster_dark.clone(),
            ThemeType::Light => self.missing_poster_light.clone(),
        }
    }

    /// Get an image without a hint.
    #[allow(unused)]
    pub(crate) fn image(&self, id: &ImageV2) -> Option<image::DynamicImage> {
        let key = ImageKey {
            id: id.hash(),
            hint: ImageHint::Raw,
        };
    ...
    pub(crate) fn missing_banner(&self) -> image::DynamicImage {
        self.missing_banner.clone()
    }

    /// Get a placeholder image for a missing screencap.
    pub(crate) fn missing_screen_capture(&self) -> image::DynamicImage {

...
impl UpdateMovie {
    ...
    pub(crate) fn new_movie(self) -> Movie {
        let earliest_releases = build_earliest_releases(&self.release_dates);
    ...
    pub(crate) fn merge_into(self, movie: &mut Movie) {
        let earliest_releases = build_earliest_releases(&self.release_dates);

        movie.title = self.title;
        movie.release_date = self.release_date;
        movie.overview = self.overview;
        movie.graphics.merge_from(self.graphics);
        movie.remote_id = Some(self.remote_id);
        movie.release_dates = self.release_dates;
        movie.earliest_releases = earliest_releases;
    }
}
...
impl UpdateSeries {
    /// Construct a new series from a series update.
    pub(crate) fn new_series(self) -> Series {
        Series {
            id: self.id,
            title: self.title,
    ...
    pub(crate) fn merge_into(self, series: &mut Series) {
        series.title = self.title;
        series.first_air_date = self.first_air_date;
        series.overview = self.overview;
        series.graphics.merge_from(self.graphics);
        series.remote_id = Some(self.remote_id);
    }
}
...
pub struct Service {
    pub(crate) paths: Arc<paths::Paths>,
    db: Database,
...
impl Service {
    ...
    pub(crate) fn insert_series(&mut self, now: &DateTime<Utc>, data: NewSeries) {
        ...
        if let Some(current) = self.db.series.get_mut(&series_id) {
            data.series.merge_into(current);
        } else {
            self.db.series.insert(data.series.new_series());
        }
    ...
    pub(crate) fn insert_movie(&mut self, now: &DateTime<Utc>, data: NewMovie) {
        ...
        if let Some(current) = self.db.movies.get_mut(&movie_id) {
            data.movie.merge_into(current);
        } else {
            self.db.movies.insert(data.movie.new_movie());
        }
